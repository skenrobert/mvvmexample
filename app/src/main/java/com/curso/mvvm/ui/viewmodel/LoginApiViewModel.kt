...
import com.curso.mvvm.domain.GetApiUseCase
...
@HiltViewModel
class LoginApiViewModel @Inject constructor( // this class is prepared for be inject with dagger hill
    ...
    fun onCreate() {
        viewModelScope.launch {//auto kill task in the case coroutine freeze
           // isLoading.postValue(true)//progress bar
            ...
            val result = getApiUseCase()
            Log.i("logapi", "desde getApiUseCase ${result}" )
...
//                //Log.i("log", "Button Pulsado ${editName.text.toString()}")
//                Log.i("logapi", "result in ramdon" )
//
